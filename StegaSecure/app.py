...
app.config['MAIL_USERNAME'] = 'manalip2134@gmail.com'
app.config['MAIL_PASSWORD'] = 'iqjr gwwx lwod fwkp'  # Use app password here, not Gmail password
app.config['MAIL_DEFAULT_SENDER'] = 'manalip2134@gmail.com'
...
def encode_secret_in_image(image_path, secret_code):
    """Encode the secret code into the image using steganography without a delimiter."""
    # Open the image
        ...
        for col in range(img.width):
            if data_index < len(binary_message):
                ...
                for i in range(3):  # Modify RGB values
                    if data_index < len(binary_message):
                        # Set the LSB of the color channels to the next bit of the message
...
def decode_message(image_path):
    ...
    if decoded_message == secret_code :
        return redirect(url_for('dashboard'))
